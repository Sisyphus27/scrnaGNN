...
def gini(array):
    ...
    array += 1e-6
    array = np.sort(array)
    index = np.array(1, array.shape[0] + 1)
    n = array.shape[0]
        ...
        for edge in list_bfs_edges:
            ...
            if (edge == flat_tree.edges[edge]['id']):
                ...
                df_pseudotime[indices] = len_pre_edges + (
                        flat_tree.edges[edge]['len'] - adata.obs.loc[indices, 'branch_lam'])
...
def add_flat_tree_cell_pos(adata, dist_scale):
    ...
    for br_id in list_branch_id:
        ...
        cells_pos[np.where(adata.obs['branch_id'] == br_id)[0], :] = [p_pos[i, :].tolist() for i in
                                                                      range(p_pos.shape[0])]
...
def add_stream_sc_pos(adata, root='S0', dist_scale=1, dist_pctl=95, preference=None):
    ...
    for edge in dict_edge_shift_dist.keys():
        ...
        np.random.seed(100)
        cells_pos_y = node_pos_st[1] + dist_scale * adata.obs.iloc[id_cells,]['branch_dist'] * np.random.choice([1, -1],
                                                                                                                size=
                                                                                                                id_cells.shape[
                                                                                                                    0])
            ...
            for pt_i in pt:
                if (((pt_i - s_win / 2.0) >= 0) & ((pt_i + s_win / 2.0) <= len_edge_i)):
                    ...
                    for es in list_edges_st:
                        ...
                        if (es == br_id_es):
                            n_cells_on_es = n_cells_on_es + len(
                                lam_es[(lam_es > 0) & (lam_es <= abs(pt_i - s_win / 2.0))])
                        else:
                            n_cells_on_es = n_cells_on_es + len(
                                lam_es[(lam_es >= (len_es - abs(pt_i - s_win / 2.0))) & (lam_es < len_es)])
                    list_n_cells_pt.append(n_cells_pt_i + n_cells_on_es)
                elif (((pt_i - s_win / 2.0) >= 0) & ((pt_i + s_win / 2.0) > len_edge_i)):
                    n_cells_pt_i = len(
                        lam_edge_i_cl[(lam_edge_i_cl >= (pt_i - s_win / 2.0)) & (lam_edge_i_cl <= len_edge_i)])
                    n_cells_on_ed = 0
                    for ed in list_edges_ed:
                        ...
                        if (ed == br_id_ed):
                            n_cells_on_ed = n_cells_on_ed + len(
                                lam_ed[(lam_ed > 0) & (lam_ed <= (pt_i + s_win / 2.0 - len_edge_i))])
                        else:
                            n_cells_on_ed = n_cells_on_ed + len(
                                lam_ed[(lam_ed >= (len_ed - (pt_i + s_win / 2.0 - len_edge_i))) & (lam_ed < len_ed)])
...
def construct_bfs_flat_tree(adata, dict_win_ncells, dict_edge_shift_dist, max_width, cell_labels_ordered,
                            root_node=None):
    flat_tree = adata.uns['flat_tree']
    if (root_node is None):
        root_node = list(flat_tree.nodes())[0]
    ...
    for edge_i in list_bfs_edges:
        ...
        df_ncells_edge_i_cumsum_norm.iloc[:, 1:] = max_width * df_ncells_edge_i_cumsum.iloc[:, 1:] / float(
            max_ncells_cumsum)
        ...
        df_coord_y_up = df_ncells_edge_i_cumsum_norm.iloc[:, 1:].subtract(
            df_ncells_edge_i_cumsum_norm.iloc[:, -1] / 2.0, axis=0)
...
def fill_im_array(dict_im_array, df_bins_gene, flat_tree, df_base_x, df_base_y, df_top_x, df_top_y, xmin, xmax, ymin,
                  ymax, im_nrow, im_ncol, step_w, dict_shift_dist, id_wins, edge_i, cellname, id_wins_prev, prev_edge):
    pad_ratio = 0.008
...
def cal_stream_polygon_numeric(adata, dict_ann, root='S0', preference=None, dist_scale=0.9,
    ...
    while len(dfs_nodes_copy) > 1:
        ...
        prev_node = dict_tree[node]['prev']
        if (node in leaves):
            dict_shift_dist[(prev_node, node)] = -(float(1) / dist_scale) * (num_nonroot_leaf - 1) / 2.0 + id_leaf * (
                    float(1) / dist_scale)
            id_leaf = id_leaf + 1
        else:
            next_nodes = dict_tree[node]['next']
            ...
            for edge_i in bfs_edges:
                ...
                if (flat_tree.degree(root_node) > 1):
                    if (edge_i == bfs_edges[0]):
                        id_wins = [0, 1]
                        im_array = fill_im_array(im_array, df_bins_ann, flat_tree, df_base_x, df_base_y, df_top_x,
                                                 df_top_y, xmin, xmax, ymin, ymax, im_nrow, im_ncol, step_w,
                                                 dict_shift_dist, id_wins, edge_i, cellname, id_wins_prev, prev_edge)
                    id_wins = id_wins_all
                    if (edge_i[0] == root_node):
                        ...
                        id_wins_prev = [i for i, x in enumerate(df_bins_cumsum_norm.loc['edge', :]) if
                                        x[0] == prev_edge]
                    im_array = fill_im_array(im_array, df_bins_ann, flat_tree, df_base_x, df_base_y, df_top_x, df_top_y,
                                             xmin, xmax, ymin, ymax, im_nrow, im_ncol, step_w, dict_shift_dist, id_wins,
                                             edge_i, cellname, id_wins_prev, prev_edge)
                else:
                    id_wins = id_wins_all
                    if (edge_i[0] != root_node):
                        prev_edge = (dict_tree[edge_i[0]]['prev'], edge_i[0])
...
def cluster_embed_union(embed, pred_cluster):
    res = list()
    cluster = set(pred_cluster)
    for i in cluster:
        index = np.where(pred_cluster == i)
        tmp = np.mean(embed[index], axis=0)
        res.append(tmp)
        del tmp
    return np.array(res)


def feature2adj(x_embed, retain_weights=True, neighborhood_factor=0.05):
    neighborhood_size_tmp = neighborhood_factor if neighborhood_factor > 1 else round(
        x_embed.shape[0] * neighborhood_factor)
    neighborhood_size = neighborhood_size_tmp - 1 if neighborhood_size_tmp == x_embed.shape[
        0] else neighborhood_size_tmp
    neighborhood_size = 1 if neighborhood_size == 0 else neighborhood_size
    ...
    return adj_return, adj_train, edgeList, A
...
def embed2swift(x_embed, y_label, dateset_name="default", dataset_dir="./"):
    current_path = os.getcwd()
    print(current_path)

    x_norm = cluster_embed_union(x_embed, y_label)
    adj_return, adj_train, edgeList, A = feature2adj(x_norm)
    adj_norm = preprocess_graph(adj_return)
    B = adj_norm.to_dense().numpy()
    df1 = pd.DataFrame(B)
    df = pd.DataFrame(A)
    current_path_to = dataset_dir + dateset_name + ".csv"
    df.to_csv(dataset_dir + dateset_name + ".csv", sep='\t', header=False)
    ...
    df = pd.DataFrame(A)
    current_path_to = dataset_dir + dateset_name + ".csv"
    df.to_csv(dataset_dir + dateset_name + "_unnorm.csv", sep='\t', header=False)


embed = np.random.randn(4000, 300)
pred = np.zeros((4000,))
pred[2001] = 1
xx = embed2swift(embed, pred)
print(xx)
print(0)
