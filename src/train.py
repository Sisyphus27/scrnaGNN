...
def filter_data(X, highly_genes=500):
    ...
    # feature_subset = np.ones(len(X.shape[1]),dtype=bool)
    # feature_subset=
    adata = ad.AnnData(X)
    # adata = sc.AnnData(X)
...
def evaluate(model, dataloader, n_clusters=None, plot=False, save=False, use_cpu=False,
             cluster_params={}):
    """
...
def train(model, optim, n_epochs, dataloader, n_clusters, plot=False, save=False,
          use_cpu=False,
          cluster_params={}):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        model.train()
        for input_nodes, output_nodes, blocks in dataloader:
            input_nodes, output_nodes = input_nodes.to(device), output_nodes.to(device)
            ...
            adj_logits, x = model.forward(blocks, input_features)

            loss = norm * BCELoss(adj_logits, adj,
        ...
        if plot == False:
            ...
            score, _ = evaluate(model, dataloader, n_clusters,
                                 use_cpu=use_cpu, cluster_params=cluster_params)
...
def get_device(use_cpu=False):
    ...
    elif mps.is_available() and not use_cpu:
