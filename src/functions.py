...
try:
    ...
    try:
        ...
        if 'Windows' in platform.system():
            os.environ["R_HOME"] = 'C:/Program Files/R/R-4.2.2/'  # Your R version here 'R-4.0.3'
            os.environ["PATH"] = "C:/Program Files/R/R-4.2.2/" + ";" + os.environ["PATH"]
        import rpy2.robjects as robjects
    except OSError:
        raise (e)
...
def read(file_name, file_path=None, file_format=None, delimiter='\t', workdir=None, file_sample='barcodes.tsv',
    ...
    assert (file_format in ['tsv', 'txt', 'tab', 'data', 'csv', 'mtx', 'h5ad', 'pkl',
                            'pklz']), "file_format must be one of ['tsv','txt','tab','data','csv','mtx','h5ad','pkl','pklz']"
...
def add_metadata(adata, file_name, delimiter='\t', file_path=''):
    ...
    # record every cluster and its color in adata.uns
    _fp = lambda f: os.path.join(file_path, f)
    ...
    if ('label_color' in df_metadata.columns):
        adata.uns['label_color'] = pd.Series(data=df_metadata.label_color.tolist(),
                                             index=df_metadata.label.tolist()).to_dict()
    else:
...
def select_variable_genes(adata, loess_frac=0.01, percentile=95, n_genes=None, n_jobs=multiprocessing.cpu_count(),
    ...
    with multiprocessing.Pool(processes=n_jobs) as pool:
        dist_point_to_curve = pool.starmap(project_point_to_curve_distance,
                                           [(XP, mat_p[i,]) for i in range(XP.shape[0])])
        ...
        for i, ann in enumerate(color):
            if (n_components == 3):
                if (is_string_dtype(df_plot[ann])):
                    ...
                    ax_i.scatter(df_plot_shuf['Dim' + str(comp1 + 1)], df_plot_shuf['Dim' + str(comp2 + 1)],
                                 df_plot_shuf['Dim' + str(comp3 + 1)],
                                 c=colors_sns, alpha=alpha, linewidth=0,
                                 zorder=-1)
                    ax_i.legend(legend_sns, labels_sns, bbox_to_anchor=(1.08, 0.5), loc='center left',
...
def plot_branches(adata, n_components=None, comp1=0, comp2=1, comp3=2, key_graph='epg',
    ...
    if (plotly):
        if (n_components == 3):
            ...
            fig.update_layout(legend={'itemsizing': 'constant'}, width=500, height=500,
                              scene=dict(xaxis_title='Dim' + str(comp1 + 1), yaxis_title='Dim' + str(comp2 + 1),
                                         zaxis_title='Dim' + str(comp3 + 1),
                                         aspectmode='cube'))
        else:
        ...
        if (n_components == 3):
            ...
            if (show_text):
                for node_i in epg.nodes():
                    ax_i.text(epg_node_pos[node_i][comp1], epg_node_pos[node_i][comp2], epg_node_pos[node_i][comp3],
                              node_i,
    ...
    if (init_nodes_pos is None):
        ...
        if (clustering == 'ap'):
            ...
            ap = AffinityPropagation(damping=damping, random_state=42,
                                     preference=np.percentile(-euclidean_distances(input_data, squared=True),
                                                              pref_perc)).fit(input_data)
...
def elastic_principal_graph(adata, epg_n_nodes=50, incr_n_nodes=30, epg_lambda=0.02, epg_mu=0.1,
    ...
    if ('params' not in adata.uns_keys()):
        adata.uns['params'] = dict()
    adata.uns['params']['epg'].update(
        {'epg_alpha': epg_alpha, 'epg_lambda': epg_lambda, 'epg_mu': epg_mu, 'epg_trimmingradius': epg_trimmingradius,
         'epg_beta': epg_beta})
...
def prune_elastic_principal_graph(adata, epg_collapse_mode='PointNumber', epg_collapse_par=5,
    ...
    input_data = adata.obsm['X_dr']
    epg_obj_collapse = ElPiGraph.CollapseBrances(X=input_data, TargetPG=epg_obj[0], Mode=epg_collapse_mode,
                                                 ControlPar=epg_collapse_par, **kwargs)
...
def extend_elastic_principal_graph(adata, epg_ext_mode='QuantDists', epg_ext_par=0.5, epg_trimmingradius=None,
    ...
    epg_obj_extend = ElPiGraph.ExtendLeaves(X=input_data,
                                            TargetPG=epg_obj[0],  # TargetPG = epg_obj[0],
                                            TrimmingRadius=epg_trimmingradius,
                                            Mode=epg_ext_mode,
...
def plot_flat_tree(adata, color=None, dist_scale=1,
    ...
    df_plot = pd.DataFrame(index=adata.obs.index, data=adata.obsm['X_spring'],
                           columns=['FlatTree' + str(x + 1) for x in range(adata.obsm['X_spring'].shape[1])])
    for ann in color:
        df_plot[ann] = dict_ann[ann]
    df_plot_shuf = df_plot.sample(frac=1, random_state=100)
...
def plot_visualization_2D(adata, method='umap', n_neighbors=50, nb_pct=None, perplexity=30.0, color=None,
    ...
    if (plotly):
        ...
        for i, ann in enumerate(color):
            ...
            if (save_fig):
                ...
                plt.savefig(os.path.join(file_path_S, 'stream_sc_' + slugify(ann) + '.' + fig_format), pad_inches=1,
                            bbox_inches='tight')
                plt.close(fig)
...
def plot_stream(adata, root='S0', color=None, preference=None, dist_scale=0.9,
    ...
    for ann in color:
        if (is_string_dtype(dict_ann[ann])):
            if (not ((ann + '_color' in adata.uns_keys()) and (
                    set(adata.uns[ann + '_color'].keys()) >= set(np.unique(dict_ann[ann]))))):
                ...
                plt.close(fig)
                colors_sns_scaled = (255 * colors_sns).astype(int)
                adata.uns[ann + '_color'] = {tmp[ann][i]: '#%02x%02x%02x' % (
                    colors_sns_scaled[i][0], colors_sns_scaled[i][1], colors_sns_scaled[i][2])
        ...
        if (save_fig):
            ...
            if (not os.path.exists(file_path_S)):
                os.makedirs(file_path_S)
            plt.savefig(os.path.join(file_path_S, 'stream_' + slugify(ann) + '.' + fig_format), pad_inches=1,
                        bbox_inches='tight')
            plt.close(fig)
...
def detect_transition_markers(adata, marker_list=None, cutoff_spearman=0.4, cutoff_logfc=0.25, percentile_expr=95,
    ...
    if (use_precomputed and ('scaled_marker_expr' in adata.uns_keys())):
        ...
        input_markers_expressed = np.array(input_markers)[
            np.where((df_sc[input_markers] > 0).sum(axis=0) > min_num_cells)[0]].tolist()
    ...
    for edge_i in bfs_edges:
        ...
        for markername in input_markers_expressed:
            ...
            if (diff_initial_final > 0):
                logfc = np.log2(max(values_final.mean(), values_initial.mean()) / (
                        min(values_final.mean(), values_initial.mean()) + diff_initial_final / 1000.0))
            else:
                logfc = 0
        ...
        if (df_stat_pval_qval.shape[0] == 0):
            ...
            dict_tg_edges[edge_i_alias] = df_stat_pval_qval[
                (abs(df_stat_pval_qval.stat) >= cutoff_spearman)].sort_values(['qval'])
            dict_tg_edges[edge_i_alias].to_csv(
                os.path.join(file_path, 'transition_markers_' + edge_i_alias[0] + '_' + edge_i_alias[1] + '.tsv'),
                sep='\t', index=True)
...
def detect_de_markers(adata, marker_list=None, cutoff_zscore=1, cutoff_logfc=0.25, percentile_expr=95, n_jobs=1,
    ...
    for pair_i in pairs_branches:
        pair_i_alias = ((dict_node_state[pair_i[0][0]], dict_node_state[pair_i[0][1]]),
                        (dict_node_state[pair_i[1][0]], dict_node_state[pair_i[1][1]]))
        ...
        if (df_cells_sub1.shape[0] > 20 and df_cells_sub2.shape[0] > 20):
            df_de_pval_qval = pd.DataFrame(columns=['z_score', 'U', 'logfc', 'mean_up', 'mean_down', 'pval', 'qval'],
                                           dtype=float)
            for markername in input_markers_expressed:
                ...
                if (diff_mean > 0):
                    logfc = np.log2(max(np.mean(sub1_values), np.mean(sub2_values)) / (
                            min(np.mean(sub1_values), np.mean(sub2_values)) + diff_mean / 1000.0))
                else:
                    logfc = 0
                if (logfc > cutoff_logfc):
                    df_de_pval_qval.loc[markername] = np.nan
                    df_de_pval_qval.loc[markername, ['U', 'pval']] = mannwhitneyu(sub1_values, sub2_values,
            ...
            if (df_de_pval_qval.shape[0] == 0):
                ...
                dict_de_greater[pair_i_alias] = df_de_pval_qval[(abs(df_de_pval_qval['z_score']) > cutoff_zscore) &
                                                                (df_de_pval_qval['z_score'] > 0)].sort_values(
                    ['z_score'], ascending=False)
                dict_de_greater[pair_i_alias].to_csv(os.path.join(file_path,
                                                                  'de_markers_greater_' + pair_i_alias[0][0] + '_' +
                                                                  pair_i_alias[0][1] + ' and ' \
                                                                  + pair_i_alias[1][0] + '_' + pair_i_alias[1][
                                                                      1] + '.tsv'), sep='\t', index=True)
                ...
                dict_de_less[pair_i_alias].to_csv(
                    os.path.join(file_path, 'de_markers_less_' + pair_i_alias[0][0] + '_' + pair_i_alias[0][1] + ' and ' \
    ...
    for sub_edges_i in dict_de_greater.keys():
        ...
        if ('z_score' in dict_de_greater[sub_edges_i].columns):
            if (not dict_de_greater[sub_edges_i].empty):
                ...
                pos_greater = np.arange(
                    dict_de_greater[sub_edges_i].iloc[:num_markers, :].shape[0]) - 1  # the bar centers on the y axis
            else:
            ...
            if (not dict_de_less[sub_edges_i].empty):
                ...
                pos_less = np.arange(
                    dict_de_less[sub_edges_i].iloc[:num_markers, :].shape[0]) - 1  # the bar centers on the y axis
            else:
            ...
            if (save_fig):
                plt.savefig(os.path.join(fig_path, 'de_markers_' + sub_edges_i[0][0] + '_' + sub_edges_i[0][1] + ' and ' \
                                         + sub_edges_i[1][0] + '_' + sub_edges_i[1][1] + '.pdf'), pad_inches=1,
                            bbox_inches='tight')
                plt.close(fig)
        else:
            if (not dict_de_greater[sub_edges_i].empty):
...
def detect_leaf_markers(adata, marker_list=None, cutoff_zscore=1., cutoff_pvalue=1e-2, percentile_expr=95, n_jobs=1,
            ...
            if (abs(zscores)[abs(zscores) > cutoff_zscore].shape[0] >= 1):
                ...
                if (kurskal_pvalue < cutoff_pvalue):
                    ...
                    if (all(cand_conover_pvalues < cutoff_pvalue)):
                        ...
                        df_leaf_markers.loc[marker, ['zscore', 'H_statistic', 'H_pvalue']] = [cand_zscore,
                                                                                              kurskal_statistic,
                                                                                              kurskal_pvalue]
                        df_leaf_markers.loc[marker, cand_conover_pvalues.index] = cand_conover_pvalues
    df_leaf_markers.rename(columns={x: dict_node_state[x[0]] + dict_node_state[x[1]] + '_pvalue' for x in leaf_edges},
                           inplace=True)
    ...
    for x in leaf_edges:
        ...
        dict_leaf_markers[x_alias].to_csv(
            os.path.join(file_path, 'leaf_markers' + x_alias[0] + '_' + x_alias[1] + '.tsv'), sep='\t', index=True)
            ...
            if (abs(zscores)[abs(zscores) > cutoff_zscore].shape[0] >= 1):
                ...
                if (kurskal_pvalue < cutoff_pvalue):
                    ...
                    if (all(cand_conover_pvalues < cutoff_pvalue)):
                        ...
                        df_markers.loc[marker, ['zscore', 'H_statistic', 'H_pvalue']] = [cand_zscore, kurskal_statistic,
                                                                                         kurskal_pvalue]
...
def map_new_data(adata_ref, adata_new, use_radius=False):
    ...
    assert (method in ['mlle', 'umap',
                       'pca']), "'%s' is not supported. Please use one of 'mlle','umap', and 'pca' for dimension reduction." % method
    if (feature == 'var_genes'):
...
def save_web_report(adata, n_markers=5, file_name='stream_web_report', preference=None,
    ...
    **kwargs: additional arguments to plot_stream()
    ...
    for edge_i in flat_tree.edges():
        ...
        df_coord_curve_i = pd.DataFrame(branch_i_pos)
        df_coord_curve_i.to_csv(os.path.join(reportdir,
                                             'coord_curve_' + dict_nodes_label[edge_i[0]] + '_' + dict_nodes_label[
                                                 edge_i[1]] + '.csv'), sep='\t', index=False)
    ...
    for ns in list_node_start:
        ...
        plot_stream(adata, root=ns, preference=preference, save_fig=True, fig_path=reportdir, fig_format='png',
                    **kwargs)
    ...
    for edge_i in adata.uns['transition_markers'].keys():
        ...
        df_tg_i.to_csv(os.path.join(file_path, 'transition_markers_' + edge_i[0] + '_' + edge_i[1] + '.tsv'), sep='\t',
                       index=True)
    ...
    for pair_i in adata.uns['de_markers_greater'].keys():
        ...
        df_de_i_greater.to_csv(
            os.path.join(file_path, 'de_markers_greater_' + pair_i[0][0] + '_' + pair_i[0][1] + ' and ' \
    ...
    for leaf_i in adata.uns['leaf_markers'].keys():
        ...
        df_lg_i.to_csv(os.path.join(file_path, 'leaf_markers' + leaf_i[0] + '_' + leaf_i[1] + '.tsv'), sep='\t',
                       index=True)
    ...
    for root in list_node_start:
        ...
        for edge_i in adata.uns['stream_' + root]['edges'].keys():
            ...
            df_edge_pos.to_csv(os.path.join(reportdir, root,
                                            'subway_coord_line_' + dict_nodes_label[edge_i[0]] + '_' + dict_nodes_label[
                                                edge_i[1]] + '.csv'), sep='\t', index=False)
        for g in gene_list:
            gene_expr = df_gene_expr[g].copy()
            df_subwaymap_gene_expr = pd.Series(gene_expr).sample(frac=1, random_state=100)
            df_subwaymap_coord_cells_expr = pd.concat([df_subwaymap_coord_cells[['D0', 'D1']], df_subwaymap_gene_expr],
                                                      axis=1)
            df_subwaymap_coord_cells_expr.to_csv(os.path.join(reportdir, root, 'subway_coord_' + slugify(g) + '.csv'),
                                                 sep='\t')
